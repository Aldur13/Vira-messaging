...
export default function ChannelList() {
  ...
  return (
    <aside className="w-56 flex-shrink-0 flex flex-col border-r border-white/10 backdrop-blur-sm" style={{ background: 'rgba(255, 255, 255, 0.02)' }}>
      {/* Space header */}
      <div className="px-3 py-3 border-b border-white/10 flex-shrink-0">
        <button onClick={() => selectServer('')}
                className="flex items-center gap-1.5 text-white/60 hover:text-blue-300 text-[11px] font-600 cursor-pointer transition-colors mb-2">
          <ArrowLeft size={11} strokeWidth={2.5} />
          All Spaces
        </button>
        <p className="text-sm font-700 text-white truncate">{server?.name}</p>
        <div className="flex items-center gap-1 mt-1 text-[10px] font-600 text-blue-300">
      ...
      <div className="flex-1 overflow-y-auto py-2">
        ...
        <div className="px-2 mb-2">
          <div className="flex items-center justify-between px-2 py-1.5">
            <span className="text-[10px] font-700 uppercase tracking-[0.8px] text-white/30">Rooms</span>
            <Tooltip label="Add Room" side="right">
              <button onClick={() => setShowCreate(true)}
                      className="text-white/50 hover:text-blue-300 cursor-pointer transition-colors">
                ...
                return (
                  <button key={ch.id} onClick={() => selectChannel(ch.id)}
                    className={clsx(
                      ...
                      active ? 'text-blue-300 font-600 bg-white/10' : 'text-white/60 hover:text-white hover:bg-white/5',
                    )}
                  >
        ...
        {voChs.length > 0 && (
          <div className="px-2 mt-2">
            <div className="flex items-center justify-between px-2 py-1.5">
              <span className="text-[10px] font-700 uppercase tracking-[0.8px] text-white/30">Voice</span>
            </div>
            {voChs.map(ch => {
                ...
                <div key={ch.id}>
                  <button onClick={() => selectChannel(ch.id)}
                    className={clsx(
                      ...
                      active ? 'text-green-400 font-600 bg-white/10' : 'text-white/60 hover:text-white hover:bg-white/5',
                    )}
                  >
                    <span className={clsx('w-1.5 h-1.5 rounded-full flex-shrink-0', active ? 'bg-green-400' : 'bg-white/20')} />
                  ...
                  {active && members.slice(0,3).map(m => (
                    <div key={m.userId} className="flex items-center gap-2 pl-8 pr-3 py-0.5">
                      ...
                      <span className="text-[11px] text-white/60 truncate">{m.user.username}</span>
                    </div>
                  ))}
                </div>
              )
